			...
			<tbody id="list">
				<c:forEach varStatus="i" var="list" items="${Roomlist}">
					<tr class="active">
						...
						<td>${list.rmprice}</td>
						<td>${list.vprice}</td>
						<td><button class="btn btn-mini btn-info" type="button" onclick="
	 ...
	 if(matchResult==true){
		 ...
		 if(r==true){
			 $.ajax({
				  type:'post',
                dataType: 'json',
                url:'http://localhost:8080/KCSJ/Emp/VipYuDing.do?vno='+vno+'&vcard='+vcard+'&rmno='+rmno,
...
function MyYuDing(rmno,rtype){
		  ...
		  if(matchResult==true){      		 
       		...
       		if (r==true){
       	     $.ajax({
	  				  type:'post',
	                  dataType: 'json',
	                  url:'http://localhost:8080/KCSJ/Emp/yuding.do?name='+name+'&idcard='+idcard+'&tel='+tel+'&rmno='+rmno,
	                  success:function(data){
					 var objs = eval(data); 
	                	if(objs.result==0){
	                		alert("预定成功");
	...
	<div id="yuding" class="form-group">
	 <label class="" for="inputSuccess1">预定房间号 </label>
		 <input type="text" name="roomid" id="croomid" readonly />
	 <label class="control-label" for="inputSuccess1">房间类型 </label>
		 <input type="text" name="type" id="idtype" readonly />
	 <label class="control-label" for="inputSuccess1">客户姓名 </label>
		 <input type="text" name="name" id="cname" />
	 <label class="control-label" for="inputSuccess1">客户证件号</label>
		 <input type="text" name="idcard" id="cidcard"/>
	 <label class="control-label" for="inputSuccess1">联系方式</label>
		 <input type="text" name="tel" id="ctel" />
	 <span style="font-size: 12px; color: red; position: absolute; left: 400px; top: 100px;" id="dialogs"> </span> <br> 
		 <input type="button" class="btn btn-primary btn-warning" onclick="check()" value="确认预定" /> 
		 <input type="button" class="btn btn-primary btn-danger" value="关闭" onclick="closeDiv()">
	</div>
  	                ...
  	                success:function(data){
  	                	...
  	                	$tbody.empty();  
  	                	            for (var j = 0; j < data.length; j++) { 
  	                	            	var table="<tr class='warning'><td>"+data[j].rmno+"</td><td>"+data[j].rmtype+"</td><td>"+data[j].rmprice
  	                	            	+"</td><td>"+data[j].vprice +"</td>"+"<td><a onclick='MyYuDing("+data[j].rmno+")'>预定</a></td></tr>";
  	                	            	$tbody.append(table);
  	                	            }
 		...
 		$.ajax({
               ...
               success:function(data){
               	...
               	$tbody.empty();  
               	         for (var j = 0; j < data.length; j++) { 
               	           var table="<tr class='warning'><td>"+data[j].rmno+"</td><td>"+data[j].rmtype+"</td><td>"+data[j].rmprice
               	            	+"</td><td>"+data[j].vprice +"</td>"+"<td><a onclick='tuiding("+data[j].rmno+")'>退定</a></td></tr>";
               	            	$tbody.append(table);
               	            }
		 ...
		 var r=confirm("确认退订？？");
    		if (r==true){
    	     $.ajax({
	  				  type:'post',
	                  dataType: 'json',
	                  url:'http://localhost:8080/KCSJ/Emp/tuiding.do?rmno='+rmno,
	                  success:function(data){
					 var objs = eval(data); 
	                	if(objs.result==0){
	                		alert("退订成功");
		...
		$.ajax({
              ...
              success:function(data){
              	...
              	$tbody.empty();  
              	         for (var j = 0; j < data.length; j++) { 
              	           var table="<tr class='warning'><td>"+data[j].rmno+"</td><td>"+data[j].rmtype+"</td><td>"+data[j].rmprice
              	                   	+"</td><td>"+data[j].vprice +"</td>"+"<td><a onclick='yudingruzhu(" + data[j].rmno+")'>预定入住</a></td></tr>";
						$tbody.append(table);
					}
