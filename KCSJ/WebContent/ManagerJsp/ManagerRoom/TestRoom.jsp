			...
			<nav class="navbar navbar-default" role="navigation">
				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
					...
					<form class="navbar-form navbar-left" role="search" action="<%=basePath%>Manager/queryRoom.do">
					  ...
					  <button  class="btn btn-mini btn-danger"  id="test">点击</button>
  		...
  		$(function(){
  			$("#test").click(function(){
  				$.ajax({
  	  				type:'GET',
  	                dataType: 'json',
  	                url:'http://localhost:8080/KeChengSheJi/Manager/Getroomlist.do',
  	                success:function(data){
  	                	...
  	                	//$tbody.empty();  
  	                	            ...
  	                	            for (var j = 0; j < data.length; j++) { 
  	                	            	...
  	                	            	var tTr="<tr class='warning'><td>"+data[j].rmno+"</td><td>"+data[j].rmtype+"</td><td>"+data[j].rmprice +"</td><td>"+data[j].vprice +"</td>"+"<td><a onclick='change("+data[j].rmno+")'>入住</a></td></tr>";
  	                	            	$tbody.append(tTr);
  	                	            }
  	                	            
  	                	             hide();
  	                	            	
  	                	            	/*$tbody.append("<tr class='warning'>").append("<td>" + data[j].rmno  
  	                	                        + "</td>").append("<td>" + data[j].rmtype  
  	                	                        + "</td>").append("<td>" + data[j].rmprice  
  	                	                        + "</td>").append("<td>" + data[j].vprice  
  	                	                        + "</td>"+"</tr>");
  	                	              )*/
  	                	          //$tbody.append("</tr>");
    	  ...
    	  function show(rmno,type,price,vprice) {
    		  ...
    		  $("#vprice1").get(0).value = vprice;
    	        ...
    	        if (sstartTime != "") {
    	            ...
    	            var info = window.open("/module/letter/daochu.action?sstartTime=" + sstartTime + "&sTime="+sTime, window_name, options);
    	            return true;
    	        } else {
    	            alert("请输入接受时间!")
    	        }
